...
const PersonalInformation = () => {
  ...
  useEffect(() => {
    const fetchCountryCodes = async () => {
      setIsLoading((prev) => ({ ...prev, countryCodes: true }))
      try {
        const response = await fetch("https://api.sentryspot.co.uk/api/jobseeker/countries")
        if (response.ok) {
          const data = await response.json()
  ...
  const handleAutoFix = async (field, content) => {
    ...
    try {
      ...
      const response = await fetch(`https://api.sentryspot.co.uk/api/jobseeker${endpoint}`, {
        method: "POST",
        headers: {
          "Content-Type": "application/json",
          Authorization: `${token}`,
  ...
  const handleContactChange = (e) => {
    const { value } = e.target
    const fullContactValue = `${selectedCountryCode} ${value.replace(/^(\+\d+\s*)?/, "")}`
...
console.log(resumeStrength,"rss");
  ...
  return (
    <div className="flex flex-col gap-3 w-full items-center md:mt-10 p-4 md:px-10">
      <h2 className="text-2xl md:text-3xl font-semibold text-white">Detail Information</h2>
                ...
                {hasCountryCode && (
                  <div className="relative w-full">
                    <div className="flex items-center">
                      ...
                      {/* Error Icon for Contact Information */}
                      {improve && hasErrors(field) && !resolvedFields[field] && (
                        <button
                          type="button"
                          className="absolute right-2 mt-2 text-red-500 hover:text-red-600 transition-colors"

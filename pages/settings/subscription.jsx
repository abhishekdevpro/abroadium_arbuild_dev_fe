    ...
    try {
      ...
      const response = await axios.post(
        ...
        {},
        {
          headers: {
            "Content-Type": "application/json",
            Authorization: token, // Use "Bearer " + token if API requires
          },
        }
      );
      ...
      if (response.status === 200) {
        toast.success("Your subscription has been canceled.");
        setStatus("Inactive"); // Immediately reflect UI
      } else {
        toast.error(
          response.data.message || "Failed to cancel the subscription."
        );
    ...
    const fetchUserProfile = async () => {
      try {
        ...
        if (response.data?.status === "success") {
          ...
          setUserData(user.personal_details);

          // Explicit boolean check
          if (user.personal_details.is_active_plan === true) {
            setStatus("Active");
          } else {
            setStatus("Inactive");
          }
              ...
              <div className="mt-6">
                ...
                <div className="flex flex-col md:flex-row justify-between items-start md:items-center mt-3">
                  ...
                  <div className="flex space-x-4">
                    ...
                    {/* Cancel button only visible if active plan */}
                    {status === "Active" && (
                      <Button
                        onClick={handleCancelSubscription}
                        className="mt-3 md:mt-0 px-4 py-2 rounded-full bg-red-600 text-white hover:bg-red-700"
                      >
                        Cancel Subscription
                      </Button>
                    )}
